...
-- use in function on order_name
-- select * from orders where order_name in ('order1','order3');

-- use alter table
alter table orders add order_price int not null;
ALTER TABLE orders modify order_price int default 0;
ALTER TABLE orders drop order_price;
DESCRIBE orders;
